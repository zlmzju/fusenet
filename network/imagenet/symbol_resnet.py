...
def get_conv(name, data, kout, kernel, stride, pad, with_relu=True):
    ...
    bn = mx.symbol.BatchNorm(name=name + '_bn', data=conv, fix_gamma=False, momentum=0.99, eps=2e-5)
...
def get_symbol(num_classes=1000, net_depth=50):
    ...
    pool1 = mx.symbol.Pooling(name='g0_pool', data=conv1, kernel=(3, 3), stride=(2, 2), pad=(0, 0), pool_type='max')

...
def get_iterator(args, kv):
    ...
    train = mx.io.ImageRecordIter(
        ...
        rand_crop=True,
        rand_mirror=True,
        max_random_scale=1.0 if args.aug_type!=0 else 0.533,   #480*0.534=256
        min_random_scale=1.0 if args.aug_type!=0 else 0.533,  #480*0.533=256
        min_img_size=256,   #ignored for 480 input
        min_crop_size=196 if args.aug_type!=0 else 256,  #area(180x180)/area(480x480)=0.14
        max_crop_size=480 if args.aug_type!=0 else 256,
        max_aspect_ratio=0.25 if args.aug_type!=0 else 0, #aspect [0.75,1.25]
        #random color jitter
        max_random_contrast=0.2 if args.aug_type!=0 else 0, #each value factor [0.8,1.2]
        max_random_illumination=10 if args.aug_type!=0 else 0, #noise addition [-10,10], 10/57.8=0.17
        random_h=5 if args.aug_type==2 else 0,
        random_s=20 if args.aug_type==2 else 0,
        random_l=20 if args.aug_type==2 else 0,
        # #others
        inter_method=9, #auto select resize method: bilinear, cubic, etc.
        seed=int(time.time()), #for retrain
    ...
    val = mx.io.ImageRecordIter(
        ...
        rand_mirror=False,        
        max_random_scale=0.533,   #480*0.534=256
        min_random_scale=0.533,  #480*0.533=256
        min_img_size=256,
        inter_method=9, #auto
        batch_size=args.batch_size,
        num_parts=kv.num_workers,
...
class Init(mx.init.Xavier):
    ...
    def _init_proj(self, _, arr):
        '''Initialization of shortcut of kenel'''
        w = np.zeros(arr.shape, np.float32)
        for i in range(w.shape[1]):
            w[i, i, ...] = 1.0/(w.shape[2]*w.shape[3]) #1.0 for (1,1), 0.25 for (2,2)
        arr[:] = w
...
#            step = max(int(epoch_size*args.lr_epoch_step), 1),
